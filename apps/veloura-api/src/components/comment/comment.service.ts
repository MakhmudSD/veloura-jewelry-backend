...
export class CommentService {
  ...
  public async createComment(memberId: ObjectId, input: CommentInput): Promise<Comment> {
    ...
    // ---- Fire notification to the correct receiver ----
    try {
      ...
      if (input.parentId && Types.ObjectId.isValid(String(input.parentId))) {
        ...
        // console.log('Reply: notifying parent owner:', receiverId);
      } else {
        // Fresh comment -> notify target owner (product/article/member)
        switch (input.commentGroup) {
          case CommentGroup.PRODUCT: {
            const product = await this.productService.findById(input.commentRefId);
            // Adjust to your product schema: prefer ownerId/memberId/authorId depending on model
        ...
        // console.log('New comment: notifying target owner:', receiverId);
      }
      ...
      // Do not throw; comment creation succeeded
    }
