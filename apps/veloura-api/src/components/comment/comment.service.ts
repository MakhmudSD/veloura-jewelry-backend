...
public async createComment(memberId: ObjectId, input: CommentInput): Promise<Comment> {
	...
	try {
		...
		if (input.parentId && Types.ObjectId.isValid(input.parentId.toString())) {
		  ...
		  // This is a top-level comment — notify owner of the target entity
	...
	// 3️⃣ If it's a reply, you can do extra logic if needed
	if (input.parentId) {
	  console.log(`New reply created under parent comment: ${input.parentId}`);
	  // You could increment reply count on parent comment here if you want
	}
  
	// 4️⃣ Update related stats
	switch (input.commentGroup) {
	  case CommentGroup.PRODUCT:
