...
export class OrderService {
  ...
  async getMyOrders(memberId: ObjectId, inquiry: OrderInquiry): Promise<Order[]> {
    ...
    const result = await this.orderModel.aggregate([
      ...
      // ✅ Correct collection name (matches OrderItemSchema collection: 'orderItems')
      {
        $lookup: {
          from: 'orderItems',
          localField: '_id',
      ...
      // ✅ Keep only rows that actually have an order item (prevents null objects without _id)
      ...
      // Attach product snapshot onto the item for GraphQL
      { $addFields: { 'orderItems.productData': '$productData' } },
      ...
      // Group back per order
      {
        $group: {
          ...
          orderItems: { $push: '$orderItems' }, // all items here have _id
        },
      },
    ...
    // Return empty array instead of throwing to avoid 500s on empty pages
    return result ?? [];
  }
